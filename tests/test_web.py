...
# Error page tests
# -----------------------------------------------------------------------
class TestErrorPage(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        return make_app(buckaroo_port=8455)

    @patch("xorq_web.handlers.get_session_entries", return_value=[])
    @patch("xorq_web.handlers.catalog_health_check")
    @patch("xorq_web.handlers.CatalogSnapshot", side_effect=RuntimeError("catalog broken"))
    def test_catalog_error_shows_error_page(self, mock_snap, mock_health, mock_session):
        from xorq_web.catalog_utils import CatalogHealthReport

        mock_health.return_value = CatalogHealthReport(
            healthy=False,
            error_type="AssertionError",
            error_message="catalog desync",
            summary="Catalog failed to load",
        )
        resp = self.fetch("/")
    ...
    @patch("xorq_web.handlers.catalog_health_check")
    @patch("xorq_web.handlers.CatalogSnapshot", side_effect=RuntimeError("catalog broken"))
    def test_catalog_error_shows_diagnostics(self, mock_snap, mock_health, mock_session):
        from xorq_web.catalog_utils import CatalogHealthReport
        ...
        mock_health.return_value = CatalogHealthReport(
            ...
            error_message="catalog desync",
            summary="Catalog failed to load",
            mitigation_steps=["Use xorq_doctor(fix=True)"],
            repo_path="/fake/repo",
        )
        resp = self.fetch("/")
        assert resp.code == 500
        body = resp.body.decode()
        assert "Diagnostics" in body or "Diagnostic" in body
        assert "What you can do" in body

    @patch("xorq_web.handlers.get_session_entries", return_value=[])
    @patch("xorq_web.handlers.catalog_health_check")
    @patch("xorq_web.handlers.CatalogSnapshot", side_effect=RuntimeError("catalog broken"))
    def test_entry_handler_error_page(self, mock_snap, mock_health, mock_session):
        from xorq_web.catalog_utils import CatalogHealthReport

        mock_health.return_value = CatalogHealthReport(healthy=False)
        resp = self.fetch("/entry/Foo")
        assert resp.code == 500
        body = resp.body.decode()
        assert "Something went wrong" in body
    ...
    @patch("xorq_web.handlers.catalog_health_check")
    @patch("xorq_web.handlers.CatalogSnapshot", side_effect=RuntimeError("catalog broken"))
    def test_runs_handler_error_page(self, mock_snap, mock_health, mock_session):
        from xorq_web.catalog_utils import CatalogHealthReport
...
# -----------------------------------------------------------------------
# Catalog health check tests
# -----------------------------------------------------------------------
class TestCatalogHealthCheck:
    def test_healthy_catalog(self):
        from xorq_web.catalog_utils import catalog_health_check

        with patch("xorq_web.catalog_utils._find_catalog_repo_path", return_value=None):
            report = catalog_health_check()
        # With no repo path found, it won't be healthy
        assert not report.healthy
        assert "Could not locate" in report.summary

    @patch("xorq_web.catalog_utils._read_catalog_yaml", return_value={"entry": [], "alias": []})
    @patch("xorq_web.catalog_utils._scan_catalog_fs")
    @patch("xorq_web.catalog_utils._find_catalog_repo_path")
    def test_broken_catalog_assertion(self, mock_path, mock_fs, mock_yaml):
        from xorq_web.catalog_utils import catalog_health_check

        mock_path.return_value = Path("/fake/repo")
        ...
        with patch("xorq.catalog.Catalog") as mock_cat:
            mock_cat.from_default.side_effect = AssertionError("desync")
            report = catalog_health_check()

        assert not report.healthy
        assert report.error_type == "AssertionError"
        assert report.is_desync
        assert report.missing_from_yaml == ["a", "b"]
